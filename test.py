...
import functools
from scipy.stats import ksone, chi2, norm
...
def significance_D(data, n, table):
    n -= 1
    print('n = ', n)
    alphaList = [0.2, 0.15, 0.10, 0.05, 0.01]
    numerator = [1.07, 1.14, 1.22, 1.36, 1.63]
    if n < 36:
        for i in range(5):
            print(i, table[n][i])
            if data < table[n][i]:
        ...
        for i in range(5):
            print(i, numerator[i] / math.sqrt(n))
            if data < numerator[i] / math.sqrt(n):
                return alphaList[i]


def significance_D_cdf(data, n):
    return 2 - 2 * ksone.cdf(data, n)
...
def significance_N_cdf(data):
    if data < 0:
        data = 0 - data
    return 2 - 2 * norm.cdf(data)


def chi2_distribution_table():
    alpha = [0.995, 0.99, 0.98, 0.975, 0.95, 0.90, 0.80,
             0.20, 0.10, 0.05, 0.025, 0.02, 0.01, 0.005, 0.001]
    df = [0] * 37
    for i in range(1, 30):
        df[i - 1] = i
...
def significance_Chi(data, k, table):
    ...
    if k < 30:
        ...
        index = round(k / 10) + 26
    for i in range(15):
        print(data, k, i, table[index][i])

        if data < table[index][i]:
            return alpha[i]


def significance_Chi_cdf(data, k):
    return 1 - chi2.cdf(data, k)
...
def Chi_square(Data, k):
    ...
    for i in range(k):
        Chi += (c[i] - mu)**2/mu
    print(Chi)
    return significance_Chi_cdf(Chi,k-1)
...
def Wald_wolfowitz_EO(Data):  # '+' for even and '-' for odd
    ...
    sigma = math.sqrt(2.0 * n1 * n2 * (2 * n1 * n2 - n1 -
                                       n2) / ((n1 + n2)**2 * (n1 + n2 - 1)))
    Z_s = (r - mu) / sigma
    return significance_N_cdf(Z_s)
...
def good_serial(Data, k):
    ...
    #    print(N)
    #print(matrix)
    return significance_Chi_cdf(U,k*(k-1))
...
# compute sum of expect
    ...
    for i in range(7):
        ...
        # compute expect of k
        if n >= 50:
            E_k[i] = 2 * ((n - k - 2) * (k**2 + 3 * k + 1)) / \
                math.factorial(k + 3)
        else:
            E_k[i] = 2 * ((k**2 + 3 * k + 1) * n - (k**3 + 3 *
                                                    k**2 - k - 4))/math.factorial(k + 3)
        s_E += E_k[i]
    E_k[7] = (2 * n - 1) / 3 - s_E
# compute x
    for i in range(8):
    ...
    #print(E_k, R_k, x_k)
    return significance_Chi_cdf(U,7)
...
SM = simulation_random.SeedAndMul(5678, 1234)
